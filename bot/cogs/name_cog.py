...
class NameCog(commands.Cog):
    ...
    async def username(self, ctx, lim:int=30):
        ...
        embed = discord.Embed(title=op,description='That sounds cool :',color=discord.Color.dark_magenta())
    ...
    async def joe_username(self, ctx, lim:int=4):
        ...
        embed = discord.Embed(title=op,description='That sounds cool, cool, cool.. Right.', color=discord.Color.gold())
        await ctx.send(embed=embed)
